...
Este ejemplo muestra como crear un temporizador para generar dos señales de reloj internas __foX__ de baja frecuencia 1KHz utilizando el cristal oscilador externo de 27MHz conectado al pin 52 (CLK) y tambien el oscilador interno que dispone la FPGA configurado mediante IPCore. La salida del circuito destellara dos luces __soX__ a 1Hz (20% encendido) que trabajan de forma independiente. Adcionalmente se creara un circuito de control de estado para una salida digital (LED6) utilizando un pulsador mecanico (S1) considerando el efecto de reboote (debounce).  
